...
public class Alarms {
    ...
    public static final String ALARM_KILLED = "my_alarm_killed";

    // Extra in the ALARM_KILLED intent to indicate to the user how long the
    // alarm played before being killed.
    public static final String ALARM_KILLED_TIMEOUT = "my_alarm_killed_timeout";

    // This string is used to indicate a silent alarm in the db.
    public static final String ALARM_ALERT_SILENT = "my_silent";

    // This intent is sent from the notification when the user cancels the
    // snooze alert.
    public static final String CANCEL_SNOOZE = "my_cancel_snooze";

    // This string is used when passing an Alarm object through an intent.
    public static final String ALARM_INTENT_EXTRA = "intent.extra.my_alarm";

    // This extra is the raw Alarm object data. It is used in the
    // AlarmManagerService to avoid a ClassNotFoundException when filling in
    // the Intent extras.
    public static final String ALARM_RAW_DATA = "intent.extra.my_alarm_raw";
    ...
    private static int defaultMinutes=5;
...
//        }
        ...
        values.put(Alarm.Columns.arriveStation, alarm.arriveStation);
        values.put(Alarm.Columns.endStation,alarm.endStation);
        values.put(Alarm.Columns.linear,alarm.linear);
        values.put(Alarm.Columns.orientation,alarm.orientation);
        values.put(Alarm.Columns.startSation,alarm.startSation);
        values.put(Alarm.Columns.stationDistance,alarm.stationDistance);
        values.put(Alarm.Columns.flag,0);
        values.put(Alarm.Columns.orderId,alarm.orderId);
        values.put(Alarm.Columns.stationId,alarm.stationId);
        values.put(Alarm.Columns.linearId,alarm.linearId);
...
//                context.getSharedPreferences(OrderPromptActivity.PREFERENCES, 0);
...
//    }
    ...
    public static Cursor getSaveAlarm(ContentResolver contentResolver, String orderId) {
        Cursor cursor = contentResolver.query(
        		Alarm.Columns.CONTENT_URI,
                Alarm.Columns.ALARM_QUERY_COLUMNS,
                Alarm.Columns.orderId+"=?",new String[]{orderId}, null);
//        Alarm alarm = null;
//        if (cursor != null) {
//            if (cursor.moveToFirst()) {
//                alarm = new Alarm(cursor);
...
//        }
    ...
    public static Alarm calculateNextAlert(final Context context) {
        ...
        final String snooze =
                ...
                .getString(SettingsActivity.KEY_ALARM_TIME, SettingsActivity.DEFAULT_TIME);
        int snoozeMinutes = Integer.parseInt(snooze);
        //long now = System.currentTimeMillis()+snoozeMinutes*60*1000;
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    //System.out.println("alarm.flag"+a.flag);
                  //  System.out.println("计算下一个闹钟"+"alarm.id:"+a.id +"alarm.flag:"+a.flag);
        	        if(a.flag&&(now>calculateAlarm(a)-snoozeMinutes*60*1000)){
        	        	//System.out.println("闹钟已经触发过");
        	        	if(a.daysOfWeek.isRepeatSet()){
        	        		 Calendar c = Calendar.getInstance();
        	        	     c.setTimeInMillis(System.currentTimeMillis());
        	        	     c.set(Calendar.HOUR_OF_DAY, a.hour);
        	        	     c.set(Calendar.MINUTE, a.minutes);
        	        	     c.add(Calendar.DAY_OF_YEAR, 1);
        	        	     int addDays = a.daysOfWeek.getNextAlarm(c);
        	        	     if (addDays > 0) 
        	        	    	  c.add(Calendar.DAY_OF_WEEK, addDays);
        	        	    //  System.out.println("如果是重复闹钟则时间设置为："+c.getTime());
        	        	      a.time=c.getTimeInMillis();
        	        	}else
        	        		continue;
        	        }
        ...
        if(alarm==null)
        	System.out.println("没有下一个闹钟");
        else{
        	System.out.println("下一个闹钟:"+alarm.id+":"+calculateAlarm(alarm.hour,alarm.minutes,alarm.daysOfWeek).getTime());
        }
        return alarm;
    }
    ...
    public static void setNextAlert(final Context context) {
        if (!enableSnoozeAlert(context)) {
            ...
            if (alarm != null) {
            	//System.out.println("setNextAlert"+alarm.time);
                enableAlert(context, alarm, alarm.time);
            } else {
            	//System.out.println("disableAlert");
            	 long now=System.currentTimeMillis();
                 //long  alarmTime=atTimeInMillis-2*60*1000;
     ...
     */
    private static void enableAlert(final Context context, final Alarm alarm,
...
//                context, 0, intent, PendingIntent.FLAG_CANCEL_CURRENT);
        final PendingIntent sender = PendingIntent.getBroadcast(
                context, alarm.id, intent, PendingIntent.FLAG_CANCEL_CURRENT);
        final String snooze =
                PreferenceManager.getDefaultSharedPreferences(context)
                .getString(SettingsActivity.KEY_ALARM_TIME, SettingsActivity.DEFAULT_TIME);
        int snoozeMinutes = Integer.parseInt(snooze);
        long now=System.currentTimeMillis();
        final long  alarmTime=atTimeInMillis-snoozeMinutes*60*1000;
        ContentResolver resolver = context.getContentResolver();
        ContentValues values = new ContentValues(1);
            if(alarm.flag&&alarm.daysOfWeek.isRepeatSet()){
            	 Calendar c = Calendar.getInstance();
            	 c.setTimeInMillis(calculateAlarm(alarm));
            	System.out.println("重复闹钟设置定时任务"+c.getTime() );
            	new Timer().schedule(new TimerTask() {
					...
					public void run() {
				        ...
				        values.put(Alarm.Columns.flag, 0);
						  resolver.update(ContentUris.withAppendedId(
					                Alarm.Columns.CONTENT_URI, alarm.id), values, null, null);
							am.setRepeating(AlarmManager.RTC_WAKEUP, alarmTime, 1*60*1000, sender);
					        Alarm alarm1=getAlarm(context.getContentResolver(), alarm.id);
					      //  System.out.println("开启闹钟"+"alarm.id:"+alarm1.id +"alarm.flag:"+alarm1.flag);
            ...
            else if(alarmTime<=now&&atTimeInMillis>now){
	        		 am.setRepeating(AlarmManager.RTC_WAKEUP, now+10*1000, 1*60*1000, sender);
	        		 values.put(Alarm.Columns.flag, 1);
	        }else {
	        		am.setRepeating(AlarmManager.RTC_WAKEUP, alarmTime, 1*60*1000, sender);
	        		values.put(Alarm.Columns.flag, 1);
	        }
     ...
     * @param id Alarm ID.
     */
    static void disableAlert(Context context,int alarmId) {
    	  
        AlarmManager am = (AlarmManager)
                context.getSystemService(Context.ALARM_SERVICE);
        
        PendingIntent sender = PendingIntent.getBroadcast(
                context, alarmId, new Intent(ALARM_ALERT_ACTION),
                PendingIntent.FLAG_CANCEL_CURRENT);
        System.out.println("闹钟关闭："+alarmId);
//    	ContentResolver resolver = context.getContentResolver();
//        ContentValues values = new ContentValues(1);
//        values.put(Alarm.Columns.flag, 0);
//		  resolver.update(ContentUris.withAppendedId(
//	                Alarm.Columns.CONTENT_URI, alarmId), values, null, null);
        am.cancel(sender);
...
//        		OrderPromptActivity.PREFERENCES, 0);
//        if (id == -1) {
//            clearSnoozePreference(context, prefs);
//        } else {
...
//    };
    ...
    public static Calendar calculateAlarm(int hour, int minute,
      ...
      //  System.out.println("calculateAlarm："+c.getTime());
        return c;
    }
    ...
    private static String formatDayAndTime(final Context context, Calendar c) {
        ...
        return (c == null) ? "" : "公交提醒:"+(String)DateFormat.format(format, c);
    }
