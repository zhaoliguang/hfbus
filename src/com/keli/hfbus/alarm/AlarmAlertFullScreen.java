...
public class AlarmAlertFullScreen extends Activity {
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Alarms.ALARM_SNOOZE_ACTION)) {
            	System.out.println("Alarms.ALARM_SNOOZE_ACTION");
                snooze();
            } else if (action.equals(Alarms.ALARM_DISMISS_ACTION)) {
            	System.out.println("Alarms.ALARM_DISMISS_ACTION");
                dismiss(false);
            } else {
                Alarm alarm = intent.getParcelableExtra(Alarms.ALARM_INTENT_EXTRA);
                if (alarm != null && mAlarm.id == alarm.id) {
                	System.out.println("Alarms.ALARM_INTENT_EXTRA");
                    dismiss(true);
                }
            }
        }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        System.out.println("进入AlarmAlertFullScreen");
        mAlarm = getIntent().getParcelableExtra(Alarms.ALARM_INTENT_EXTRA);
        currentDistance=mAlarm.label;
    ...
    private void setTitle() {
        ...
        String linear=mAlarm.linear;
        String arriveStation=mAlarm.arriveStation;
        String distance=mAlarm.stationDistance;
        TextView busInfo = (TextView) findViewById(R.id.bus_info);
        System.out.println("mAlarm.label:"+mAlarm.label);
        if(mAlarm.label.equals("0")){
        	busInfo.setText(linear+"已经到达"+arriveStation+"站");
        }
        else if(mAlarm.label.equals("")){
        	busInfo.setText(linear+"还有大约"+mAlarm.stationDistance+"站到达"+arriveStation+"站");
        }
        else if(Integer.valueOf(mAlarm.label)>=Integer.valueOf(mAlarm.stationDistance)) {
        	busInfo.setText("在您预订的时间内最近的"+linear+"公交还有"+mAlarm.label+"站到达"+arriveStation+"站");
        }
        else
        	busInfo.setText(linear+"还有大约"+mAlarm.label+"站到达"+arriveStation+"站");
    }
    ...
    private void dismiss(boolean killed) {
        ...
        if (!killed) {
          ...
          //  Alarms.disableAlert(AlarmAlertFullScreen.this ,mAlarm.id);
            Alarms.setNextAlert(AlarmAlertFullScreen.this);
            stopService(new Intent(Alarms.ALARM_ALERT_ACTION));
        }
        finish();
    }
